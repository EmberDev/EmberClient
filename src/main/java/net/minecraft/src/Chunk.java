...
public class Chunk
{
    ...
    /**
     * Gets the amount of red light saved in this block (doesn't adjust for daylight)
     */
    public int getSavedLightValueRed(EnumSkyBlock blockType, int x, int y, int z)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        return storage == null ? (this.canBlockSeeTheSky(x, y, z) ? blockType.defaultLightValue : 0) : (blockType == EnumSkyBlock.Sky ? storage.getExtSkylightValue(x, y & 15, z) : (blockType == EnumSkyBlock.Block ? storage.getExtRGBBlocklightValueRed(x, y & 15, z) : blockType.defaultLightValue));
    }

    /**
     * Gets the amount of green light saved in this block (doesn't adjust for daylight)
     */
    public int getSavedLightValueGreen(EnumSkyBlock blockType, int x, int y, int z)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        return storage == null ? (this.canBlockSeeTheSky(x, y, z) ? blockType.defaultLightValue : 0) : (blockType == EnumSkyBlock.Sky ? storage.getExtSkylightValue(x, y & 15, z) : (blockType == EnumSkyBlock.Block ? storage.getExtRGBBlocklightValueGreen(x, y & 15, z) : blockType.defaultLightValue));
    }

    /**
     * Gets the amount of blue light saved in this block (doesn't adjust for daylight)
     */
    public int getSavedLightValueBlue(EnumSkyBlock blockType, int x, int y, int z)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        return storage == null ? (this.canBlockSeeTheSky(x, y, z) ? blockType.defaultLightValue : 0) : (blockType == EnumSkyBlock.Sky ? storage.getExtSkylightValue(x, y & 15, z) : (blockType == EnumSkyBlock.Block ? storage.getExtRGBBlocklightValueBlue(x, y & 15, z) : blockType.defaultLightValue));
    }
    ...
    /**
     * Sets the red light value at the coordinate. If enumskyblock is set to sky it ignores it and if its a
     * block then into the blocklightmap. Args enumSkyBlock, x, y, z, lightValue
     */
    public void setLightValueRed(EnumSkyBlock blockType, int x, int y, int z, int value)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        ...
        if (blockType == EnumSkyBlock.Sky)
        {
            return;
        }
        else if (blockType == EnumSkyBlock.Block)
        {
            storage.setExtRGBBlocklightValueRed(x, y & 15, z, value);
        }
    }

    /**
     * Sets the green light value at the coordinate. If enumskyblock is set to sky it ignores it and if its a
     * block then into the blocklightmap. Args enumSkyBlock, x, y, z, lightValue
     */
    public void setLightValueGreen(EnumSkyBlock blockType, int x, int y, int z, int value)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        ...
        {
            storage.setExtRGBBlocklightValueGreen(x, y & 15, z, value);
        }
    }

    /**
     * Sets the blue light value at the coordinate. If enumskyblock is set to sky it ignores it and if its a
     * block then into the blocklightmap. Args enumSkyBlock, x, y, z, lightValue
     */
    public void setLightValueBlue(EnumSkyBlock blockType, int x, int y, int z, int value)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        ...
        {
            storage.setExtRGBBlocklightValueBlue(x, y & 15, z, value);
        }
    }
    ...
    /**
     * Gets the amount of red light on a block taking into account sunlight
     */
    public int getBlockLightValueRed(int x, int y, int z, int baseValue)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];

        if (storage == null)
        {
            return !this.worldObj.provider.hasNoSky && baseValue < EnumSkyBlock.Sky.defaultLightValue ? EnumSkyBlock.Sky.defaultLightValue - baseValue : 0;
        }
        else
        {
            int result = this.worldObj.provider.hasNoSky ? 0 : storage.getExtSkylightValue(x, y & 15, z);
            ...
            result -= baseValue;
            int blockLight = storage.getExtRGBBlocklightValueRed(x, y & 15, z);

            if (blockLight > result)
            {
                result = blockLight;
            }
    ...
    /**
     * Gets the amount of green light on a block taking into account sunlight
     */
    public int getBlockLightValueGreen(int x, int y, int z, int baseValue)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        ...
        else
        {
            ...
            result -= baseValue;
            int blockLight = storage.getExtRGBBlocklightValueGreen(x, y & 15, z);
    ...
    /**
     * Gets the amount of blue light on a block taking into account sunlight
     */
    public int getBlockLightValueBlue(int x, int y, int z, int baseValue)
    {
        ExtendedBlockStorage storage = this.storageArrays[y >> 4];
        ...
        else
        {
            ...
            result -= baseValue;
            int blockLight = storage.getExtRGBBlocklightValueBlue(x, y & 15, z);
